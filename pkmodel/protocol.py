...
#Validation schema of input variable names and the expected type and formats
schema = Schema(
    [
        {
            "name": And(str, len), #checks name is a str, and not empty (length > 0)
            "Q_p1": And(Use(float), lambda n: n>=0), #tries to turn input into float, check if positive number
